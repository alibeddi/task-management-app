...
import "./ListTaskCard.css";

interface ListTaskCardProps {
  task: Task;
}

const ListTaskCard: React.FC<ListTaskCardProps> = ({ task }) => {
  ...
  return (
    <>
      <div
        ...
        {...attributes}
        {...listeners}
        className={`list-task-card ${isDragging ? "dragging" : ""}`}>
        <div className="list-task-inline-content">
          <div className="list-task-inline-title-desc">
            <h4 className="list-task-title">{task.title}</h4>
            {task.description && task.description !== "--" && (
              <span className="list-task-description">{task.description}</span>
            )}
          </div>
          {task.dueDate && task.dueDate !== "--" && (
            <div className="list-task-due-date">
